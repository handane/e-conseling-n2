          ...
          <div class="card shadow-2-strong" style="border-radius: 1rem;">
            <div class="card-body p-5 text-center">
              ...
              <form action="" method="POST">
                ...
                <button type="button" style="background: none;border:none;" data-bs-toggle="modal" data-bs-target="#exampleModal" data-bs-whatever="@mdo">Daftar</button><br>
                <button type="button" style="background: none;border:none;" data-bs-toggle="modal" data-bs-target="#exampleModal2" data-bs-whatever="@mdo">Lupa Password</button>
              </form>
              <!-- tanggapan -->
                  ...
                  <div class="modal-content">
                    ...
                    <form method="POST">
                      <div class="modal-body">
                        <div class="mb-3">
                          <label for="recipient-name" class="col-form-label"></label>
                          <input type="text" class="form-control mt-3" id="recipient-name" autocomplete="off" name="username_lama" placeholder="Username">
                          <input type="text" class="form-control mt-3" id="recipient-name" autocomplete="off" name="password_baru_ganti" placeholder="Password Baru">
                        </div>
                      </div>
                      <div class="modal-footer">
                        <input type="submit" class="btn btn-primary" name="lupa_password" value="Save">
                      </div>
                    </form>
                    <?php
                    // include_once("db.php");
                    if (isset($_POST["lupa_password"])) {
                      ...
                      $password_baru_ganti = $_POST['password_baru_ganti'];
                      $cek_regist = mysqli_query($conn, "SELECT * FROM pengguna_konseling WHERE username = '$username_lama'");
                      if (mysqli_num_rows($cek_regist) > 0) {
                        $update = mysqli_query($conn, "UPDATE pengguna_konseling SET
                        password = '$password_baru_ganti'
                        WHERE username = '$username_lama'");
                        if ($update) {
                          echo '<script>alert("password berhasil diganti")</script>';
                        } else {
                          echo '<script>alert("password gagal dibuat")</script>';
                        }
                      } else {
                        echo '<script>alert("Gagal, username tidak terdaftar")</script>';
                      }
                    }
                    ?>
                  </div>
