                  ...
                  <div class="table-responsive">
                    <table class="table table-hover">
                      ...
                      <tbody>
                        <?php
                        $no = 1;
                        $konsultasi = mysqli_query($conn, "SELECT * FROM konsultasi LEFT JOIN konselor USING(id_konselor) LEFT JOIN pengguna_konseling USING(id_konseling) WHERE status = 'selesai'");
                        if (mysqli_num_rows($konsultasi) > 0) {
                          while ($row = mysqli_fetch_array($konsultasi)) {
                        ?>
                            <tr>
                              <td><?php echo $no++; ?></td>
                              <td><img src="foto/<?php echo $row['foto_konselor']; ?>" alt=""> <?php echo $row['nama']; ?></td>
